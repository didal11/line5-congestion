...
Minimal browser editor for the existing `reference/rps/` Python project.
...
- browse real files and directories under `reference/rps/`
- open and edit UTF-8 text files
- create a new file by entering a path under `reference/rps/`
- save changes as commits on `notion-workspace`
- run a Python module or relative `.py` path using a standard GitHub-hosted Ubuntu runner
...
The initial default entrypoint is `src.train`, matching the RPS example's existing `python -m src.train` execution model.
...
After the secrets exist, run the `Deploy Notion IDE` workflow once. Its output contains the `workers.dev` URL. Put that HTTPS URL in a Notion `/embed` block.
...
5. Select a file under `reference/rps/`, edit, and press `Save`.
...
The Worker only permits file paths at or below `reference/rps/`. The GitHub token stays in a Cloudflare Worker secret. The editor password is separate from the GitHub token and is sent only in the `x-ide-key` request header to the same Worker origin.
