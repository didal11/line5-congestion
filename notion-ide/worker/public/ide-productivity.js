(() => {
  ...
  async function enqueueCurrentRun(){
    showError(); ensureConnected(); const entrypoint=currentEntrypoint(); if(currentBuffer()?.dirty)await saveFile(); const args=shellSplit(currentArgsText()); const command=await displayCommand(entrypoint,args);
    const d=await api("/api/queue",{method:"POST",body:JSON.stringify({entrypoint,args})});
    appendTerminal(`[QUEUE] ${command}\n[QUEUE] id ${d.item.id.slice(0,8)} · ${d.item.status}`); openPanel("queue"); await pollQueue(); updateRunButton(); return d;
  }
  ...
  async function pollQueue(){
    if(!config)return null; const data=await api("/api/queue"); const pending=data.items.filter(i=>!terminalState(i.status)).length;$("prodQueueBadge").textContent=String(pending);
    const active=data.items.find(i=>i.id===data.active_id); if(active?.request_commit_sha)await attachQueueItem(active);
    if(activeView==="queue")renderQueueData(data); return data;
  }
  ...
  function renderQueueData(data){
    if(activeView!=="queue")return;$("prodTitle").textContent="RUN QUEUE";const body=$("prodBody");body.innerHTML=`<div class="prod-row"><span class="prod-muted">Sequential server-side queue. Closing Notion does not stop queued work.</span><button id="prodClearQueue">Clear finished</button></div><div id="prodQueueList" class="prod-list"></div>`;$("prodClearQueue").onclick=()=>clearQueueFinished().catch(showError);const list=$("prodQueueList");
    const items=[...data.items].reverse();if(!items.length){list.innerHTML='<div class="prod-item prod-muted">Queue is empty.</div>';return}
    for(const item of items){const div=document.createElement("div");div.className="prod-item";const args=(item.args||[]).map(quoteArg).join(" ");div.innerHTML=`<div class="prod-row" style="margin:0"><span class="prod-state ${esc(item.status)}">${esc(item.status)}</span><span class="prod-item-title">${esc(item.entrypoint)}</span><span class="prod-queue-actions"></span></div><div class="prod-item-sub">${esc(args)}${item.run_id?`\nrun ${item.run_id}`:""}${item.error?`\n${esc(item.error)}`:""}</div>`;const actions=div.querySelector(".prod-queue-actions");if(!terminalState(item.status)){const b=document.createElement("button");b.textContent="Cancel";b.onclick=(e)=>{e.stopPropagation();cancelQueueItem(item.id).catch(showError)};actions.appendChild(b)}list.appendChild(div)}
  }
