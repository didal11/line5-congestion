...
async function connect(){showError();config=await api("/api/bootstrap",{method:"POST"});sessionStorage.setItem("notionIdeKey",$("key").value);$("connection").textContent="connected";$("repoLabel").textContent=config.owner+"/"+config.repo+" @ "+config.branch+" | run: "+config.run_root;loadedDirs.clear();$("tree").innerHTML="";await mountRoot()}
...
function newFile(){if(!config)return;const base=config.run_root||"";$("path").value=(base?base+"/":"")+"new_file.py";$("editor").value="";currentSha=null;setSelected(null);$("saveState").textContent="new file";$("path").focus();$("path").select()}
