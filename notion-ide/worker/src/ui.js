...
<div id="runRoot" class="run-help">base: reference/rps</div>
...
async function connect(){showError();config=await api("/api/bootstrap",{method:"POST"});sessionStorage.setItem("notionIdeKey",$("key").value);$("connection").textContent="connected";$("repoLabel").textContent=config.owner+"/"+config.repo+" @ "+config.branch+" | run: "+config.run_root;$("runRoot").textContent="base: "+config.run_root;loadedDirs.clear();$("tree").innerHTML="";await mountRoot()}
